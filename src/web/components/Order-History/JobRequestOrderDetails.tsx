...
import { useJobPostingOrderDetailsBuCustomer } from "../../utils/fetchEndpoints";
...
import { JobOrderHistory } from "../../lib/types/FindWorkPostAJobtypesData";
import { JobPostingHistoryItemDetails } from "./JobPostingHistoryItemDetails";

export const JobRequestOrderDetails: React.FC = () => {
  ...
  const { data: orderDetails } = useJobPostingOrderDetailsBuCustomer(
    Number(solutionistId),
    Number(orderId),
    Number(posterId)
  );

  const formatDate = orderDetails?.jobPostingOrderDetails?.createdAt
    ? format(
        new Date(orderDetails.jobPostingOrderDetails.createdAt),
          ...
          <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
            <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
              ...
              <JobPostingHistoryItemDetails
                content={
                  orderDetails?.jobPostingOrderDetails ??
                  ({} as JobOrderHistory)
                }
            ...
            <div className="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
              <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                ...
                <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                  <div className="flex justify-between items-center w-full">
                    <p className="text-base dark:text-white leading-4 text-gray-800">
                      {orderDetails?.jobPostingOrderDetails?.jobName} Total
                    </p>
                    <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                      $
                      {orderDetails?.jobPostingOrderDetails?.jobPrice?.toFixed(
                        2
                      )}
                    </p>
                  </div>
                  ...
                  <div className="flex justify-between items-center w-full">
                    ...
                    <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                      -$
                      {orderDetails?.jobPostingOrderDetails?.discountPrice?.toFixed(
                        2
                      )}{" "}
                      (
                      {orderDetails?.jobPostingOrderDetails?.discountPercent?.toFixed()}
                      %)
                    </p>
                  </div>
                </div>
                ...
                <div className="flex justify-between w-full">
                  ...
                  <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                    $
                    {orderDetails?.jobPostingOrderDetails?.subtotal?.toFixed(
                      2
                    ) ?? 0}
                  </p>
                </div>
                ...
                <div className="flex justify-between w-full">
                  ...
                  <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                    ${orderDetails?.jobPostingOrderDetails?.tax?.toFixed(2)}
                  </p>
                </div>
                ...
                <div className="flex justify-between items-center w-full">
                  ...
                  <p className="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">
                    $
                    {orderDetails?.jobPostingOrderDetails?.grandTotal?.toFixed(2)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 w-full h-full xl:w-96 flex justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col">
                ...
                <div className="flex justify-center text-gray-800 dark:text-white md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                  <p className="flex items-center gap-2">
                    ...
                    <span className="cursor-pointer text-sm leading-5">
                      {orderDetails?.posterDetails?.email}
                    </span>
                  </p>
                </div>
              </div>
                ...
                <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                  <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                    ...
                    <p className="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">
                      <Address
                        country={
                          orderDetails?.jobPostingOrderDetails?.jobCountry ?? ""
                        }
                        address={
                          orderDetails?.jobPostingOrderDetails?.jobAddress ?? ""
                        }
                        city={
                          orderDetails?.jobPostingOrderDetails
                            ?.jobCityLocation ?? ""
                        ...
                        state={
                          orderDetails?.jobPostingOrderDetails?.jobState ?? ""
                        }
                        zip={orderDetails?.jobPostingOrderDetails?.jobZip ?? ""}
                      />
                    </p>
                  </div>
            ...
            <div className="flex flex-col justify-start items-start flex-shrink-0 w-full">
              <div className="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                ...
                <div className="flex justify-start items-start flex-col space-y-2">
                  <p className="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">
                    {orderDetails?.solutionistDetails?.firstName}{" "}
                    {orderDetails?.solutionistDetails?.lastName}
                  </p>
                </div>
              </div>
            </div>
